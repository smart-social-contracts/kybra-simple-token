...
def run_tests():
    ...
    assert_equals(100000000000000000, result, "icrc1_total_supply returns initial supply")
    ...
    result = dfx_call("icrc1_balance_of", f'(record {{ owner = principal "{deployer}"; subaccount = null }})')
    assert_equals(100000000000000000, result, "deployer has initial supply")
    
    result = dfx_call("icrc1_balance_of", f'(record {{ owner = principal "{alice}"; subaccount = null }})')
    ...
    assert_true(isinstance(result, dict) and result.get("success") == True, "transfer from deployer to alice succeeds")
    ...
    assert_true(isinstance(result, dict) and result.get("success") == True, "transfer to subaccount succeeds")
    
    result = dfx_call("icrc1_balance_of", f'(record {{ owner = principal "{bob}"; subaccount = opt {subaccount_blob} }})')
    ...
    assert_true(isinstance(result, dict) and result.get("success") == False, "transfer with insufficient balance fails")
    assert_contains(result.get("error", ""), "Insufficient balance", "error message contains 'Insufficient balance'")
    ...
    assert_true(isinstance(result, dict) and result.get("success") == True, "mint in test mode succeeds")
    
    result = dfx_call("icrc1_balance_of", f'(record {{ owner = principal "{charlie}"; subaccount = null }})')
    ...
    assert_true(isinstance(result, dict) and result.get("success") == True, "charlie can transfer minted tokens")
    ...
    result = dfx_call("get_account_transactions", f'(record {{ account = record {{ owner = principal "{alice}"; subaccount = null }}; start = null; max_results = 10 : nat }})')
    assert_true("Ok" in str(result) or isinstance(result, dict), "get_account_transactions returns result")
    
    result = dfx_call("get_transactions", "(0 : nat, 10 : nat)")
    assert_true(isinstance(result, dict), "get_transactions returns dict")
    assert_true("transactions" in result or "transactions" in str(result), "get_transactions has transactions")
    
    result = dfx_call("get_top_holders", "(5 : nat)")
    assert_true(isinstance(result, list) or "address" in str(result), "get_top_holders returns holders")
